...
var (
	...
	SysConfigList          = hpapi.SysConfigList{}
	inited                 = false
	RouterBasepathDefault  = "/"
	RouterBasepathDefaults = []string{}
	Languages              = []*hpapi.LangEntry{}
)
...
func init() {
	...
	//
	SysConfigList.Insert(hpapi.SysConfig{
	...
	//
	{
		...
		for _, v := range rs {
			...
			if item.Key == "frontend_languages" {
				if langs := hpapi.LangsStringFilterArray(item.Value); len(langs) > 0 {
					Languages = []*hpapi.LangEntry{}
