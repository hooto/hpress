...
func SpecSet(spec *hpapi.Spec) {

	locker.Lock()
	defer locker.Unlock()

	if strings.Contains(spec.SrvName, "/") {
		spec.SrvName, _ = hpapi.SrvNameFilter(spec.SrvName)
	}
...
func SpecGet(modname string) *hpapi.Spec {
...
func SpecNodeModel(modname, modelName string) (*hpapi.NodeModel, error) {
	...
	return &hpapi.NodeModel{}, errors.New("Spec Not Found")
}

func SpecTermModel(modname, modelName string) (*hpapi.TermModel, error) {
...
func moduleInit() error {
	...
	if rs := store.Data.Query(q); rs.OK() {
		...
		for rs.Valid() {
			...
			if err := rs.Field("body").JsonDecode(&mod); err == nil && mod.Meta.Name != "" {
				if mod.SrvName == "" || strings.Contains(mod.SrvName, "/") {
					mod.SrvName, _ = hpapi.SrvNameFilter(rs.Field("srvname").String())
				}
				...
				for j, v2 := range mod.NodeModels {
					...
					if ft := v2.Field("title"); ft == nil {
						v2.Fields = append([]hpapi.FieldModel{
							{
								Name:   "title",
								Type:   "string",
								Length: "100",
					...
					//
					table := hpapi.NodeTableName(mod.Meta.Name, v2.Meta.Name)
					qs := store.Data.NewQueryer().
						Select("id,title,field_title").
						From(table).
						Limit(10000)

					if rs2 := store.Data.Query(qs); rs2.OK() && !rs2.NotFound() {

						num := 0

						for rs2.Valid() {
							if len(rs2.Field("field_title").String()) < 2 {

								// fmt.Println("id", rs2.Field("id").String(), rs2.Field("title").String())

								fr := store.Data.NewFilter()
								fr.And("id", rs2.Field("id").String())

								set := map[string]interface{}{
									"field_title": rs2.Field("title").String(),
								}
						...
						if n, _ := rs2.RowsAffected(); n > 0 {
							slog.Warn(fmt.Sprintf("upgrade %s/%s %d/%d", mod.Meta.Name, table, num, n))
						}
					}
				}
				...
				slog.Error(fmt.Sprintf("Module.Init(%s) Failed", rs.Field("name").String()))
			}
	...
	for _, modname := range coreModules {
		...
		if sync {
			spec.Meta.Version = hpapi.NewSpecVersion(spec.Meta.Version).Add(0, 0, 1).String()
		}

		var instResVersion hpapi.SpecVersion
		for _, mod := range Modules {
			if mod.Meta.Name == modname {
				instResVersion = hpapi.SpecVersion(mod.Meta.Version)
				break
			}
		}

		if hpapi.NewSpecVersion(spec.Meta.Version).Compare(&instResVersion) <= 0 {
			continue
		}
...
func SpecSrvRefresh(srvname string) {
	...
	if strings.Contains(srvname, "/") {
		srvname, _ = hpapi.SrvNameFilter(srvname)
	}
