...
type LynkSearchConfigBucketEntry struct {
	...
	Model            *hpapi.NodeModel `json:"model"`
}
...
func (it *NodeLynkSearchEngine) ModelSet(bukName string, model *hpapi.NodeModel) error {
...
func (it *NodeLynkSearchEngine) indexFull(active *lynkSearchBucketActive) error {
	...
	var (
		offset = hpapi.NsTextSearchCacheNodeEntry(active.config.Name, "")
		cutset = hpapi.NsTextSearchCacheNodeEntry(active.config.Name, "")
		num    = 0
	)
...
func (it *NodeLynkSearchEngine) Put(bukName string, node hpapi.Node) error {

	key := hpapi.NsTextSearchCacheNodeEntry(bukName, node.ID)
...
func (it *NodeLynkSearchEngine) Query(bukName string, q string, qs *QuerySet) hpapi.NodeList {
	...
	if err != nil {
		ls.Error = types.NewErrorMeta(hpapi.ErrCodeInternalError, err.Error())
		return ls
	}
	...
	if !rs.OK() {
		ls.Error = types.NewErrorMeta(hpapi.ErrCodeInternalError, rs.Err().Error())
		return ls
	}
	{
	...
	if len(rows) == 0 {
		ls.Error = types.NewErrorMeta(hpapi.ErrCodeInternalError, "server error")
		return ls
	}

	for _, row := range rows {
		if rs := store.DataLocal.NewReader(
			hpapi.NsTextSearchCacheNodeEntry(bukName, row.Id)).Exec(); rs.OK() {
			var node hpapi.Node
...
func lynkSearchAddDocument(node *hpapi.Node, active *lynkSearchBucketActive) {
