<script lang="ts">
  ...
  async function load() {
    try {
      const rsj = await api.get<{ kind?: string; items?: Spec[] }>('mod-set/spec-list', qry ? { qry_text: qry } : undefined)
      if (!rsj || rsj.kind !== 'SpecList' || !rsj.items || rsj.items.length < 1) {
        items = []
  ...
  function openMgr(which: 'node' | 'term' | 'action' | 'route', name: string) {
    const map = { node: NodeMgr, term: TermMgr, action: ActionMgr, route: RouteMgr }
    const titles = { node: 'Node List', term: 'Term List', action: 'Action List', route: 'Route List' }
    openModal({
      title: titles[which],
...
<div class="hpm-block-gap-column">
  <div class="d-flex flex-row align-items-center hpm-block-gap-row-sm" style="margin-bottom:8px">
    <button class="btn btn-primary" on:click={() => infoSet()}>New Module</button>
    <button class="btn btn-outline-primary" on:click={upload}>
      Install or Upgrade from Package
    </button>
    <form on:submit|preventDefault={load} class="d-inline-block ms-2">
        ...
        <tbody>
          {#each items as v (v.meta.name)}
            <tr>
              ...
              <td>
                <button class="btn btn-sm btn-outline-dark" on:click={() => openMgr('node', v.meta.name)}
                  >{v._nodeModelsNum}</button
                >
              </td>
              <td>
                <button class="btn btn-sm btn-outline-dark" on:click={() => openMgr('action', v.meta.name)}
                  >{v._actionsNum}</button
                >
              </td>
              <td>
                <button class="btn btn-sm btn-outline-dark" on:click={() => openMgr('route', v.meta.name)}
                  >{v._routesNum}</button
                >
              </td>
              <td>
              ...
              <td align="right">
                <button class="btn btn-sm btn-outline-dark" on:click={() => develop(v.meta.name)}>Develop</button>
                <button class="btn btn-sm btn-outline-dark" on:click={() => infoSet(v.meta.name)}>Setting</button>
              </td>
            </tr>
          {/each}
        </tbody>
